// `factor` expects a scalar log score. Use `observe` (or `factorMany`) when a
// distribution returns per-element additive log-probability contributions that
// should be summed into one joint score.
#load "HmcKernel.fsx"
...
let logSqrtTwoPi = Term.scalar (0.5 * log (2.0 * System.Math.PI))
    ...
    member _.For(sequence: seq<'a>, body: 'a -> ModelExpr<unit>) : ModelExpr<unit> =
        fun state ->
            ...
            while enumerator.MoveNext() do
                let (), nextState = body enumerator.Current state
                state <- nextState
...
let runLinearRegressionExample () =
    let x = Term.data (dsharp.tensor([ for i in -12 .. 12 -> float i / 6.0 ]))
    let trueAlpha = -0.4
    let trueBeta = 1.1
    let trueSigma = 0.25
    let y =
        let xTensor = Term.eval { Parameters = Map.empty } x
        scalar trueAlpha + scalar trueBeta * xTensor + scalar trueSigma * dsharp.randnLike(xTensor)
        |> Term.data

    let model =
        prob {
            let! alpha = normalLatent "alpha" 0.0 0.0 1.5
            and! beta = normalLatent "beta" 0.0 0.0 1.5
            and! sigma = positiveNormalLatent "sigma" -1.0 0.0 0.75

            let mean = alpha + beta * x
            do! observe "y" (normal mean sigma) y

            return Term.stack [ alpha; beta; sigma ]
        }
    ...
    printfn "Posterior [alpha; beta; sigma]: %A" posteriorMean
    printfn "Decoded alpha: %A" decodedMean.["alpha"]
    printfn "Decoded beta:  %A" decodedMean.["beta"]
    printfn "Decoded sigma: %A" decodedMean.["sigma"]
    printfn "Ground truth:  %A" (dsharp.tensor([ trueAlpha; trueBeta; trueSigma ]))
